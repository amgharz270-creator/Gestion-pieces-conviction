...
@section('title', 'Nouveau Mouvement - TPI Sidi Bennour')
...
<div class="admin-wrapper">
    ...
    <main class="main-content">
        ...
        <div class="page-header">
            <div>
                <a href="{{ route('mouvements.index') }}"><i class="bi bi-arrow-left"></i> Retour a la liste</a>
                <h1><i class="bi bi-arrow-left-right" style="color: var(--accent-gold); margin-right: 12px;"></i>Nouveau Mouvement</h1>
            </div>
        </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="type" class="form-select-dark {{ $errors->has('type') ? 'is-invalid' : '' }}" required>
                            ...
                            <option value="saisie" {{ old('type') == 'saisie' ? 'selected' : '' }}>Saisie</option>
                            <option value="transfert" {{ old('type') == 'transfert' ? 'selected' : '' }}>Transfert</option>
                            <option value="restitution" {{ old('type') == 'restitution' ? 'selected' : '' }}>Restitution</option>
                            <option value="destruction" {{ old('type') == 'destruction' ? 'selected' : '' }}>Destruction</option>
                            ...
                            <option value="audience" {{ old('type') == 'audience' ? 'selected' : '' }}>Audience</option>
                            <option value="retour_depot" {{ old('type') == 'retour_depot' ? 'selected' : '' }}>Retour au Dépôt</option>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="to_user_id" class="form-select-dark {{ $errors->has('to_user_id') ? 'is-invalid' : '' }}" required>
                        ...
                        @if($errors->has('to_user_id'))<div class="invalid-feedback">{{ $errors->first('to_user_id') }}</div>@endif
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="to_emplacement_id" class="form-select-dark {{ $errors->has('to_emplacement_id') ? 'is-invalid' : '' }}">
                            ...
                            <option value="{{ $empl->id }}" {{ old('to_emplacement_id') == $empl->id ? 'selected' : '' }}>
                                {{ $empl->local }} - {{ $empl->armoire }} ({{ $empl->pieces_actuelles }}/{{ $empl->capacite_max }})
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="datetime-local" name="date_mouvement" class="form-control-dark {{ $errors->has('date_mouvement') ? 'is-invalid' : '' }}" value="{{ old('date_mouvement', now()->format('Y-m-d\TH:i')) }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="datetime-local" name="date_retour_prevue" class="form-control-dark {{ $errors->has('date_retour_prevue') ? 'is-invalid' : '' }}" value="{{ old('date_retour_prevue') }}">
                        @if($errors->has('date_retour_prevue'))<div class="invalid-feedback">{{ $errors->first('date_retour_prevue') }}</div>@endif
                    </div>

                    <div class="col-12">
                        <label class="form-label">Référence Document</label>
                        <input type="text" name="document_reference" class="form-control-dark" value="{{ old('document_reference') }}" placeholder="PV, Ordonnance, Référence...">
                    </div>

                    <div class="col-12">
                        <label class="form-label">Motif</label>
                        <textarea name="motif" class="form-control-dark" rows="2" placeholder="Motif du mouvement...">{{ old('motif') }}</textarea>
                    </div>

                    <div class="col-12">
                        <label class="form-label">Observations</label>
                        <textarea name="observations" class="form-control-dark" rows="2" placeholder="Observations éventuelles...">{{ old('observations') }}</textarea>
                    </div>

                    <div class="col-12 mt-4">
                        <button type="submit" class="btn-gold"><i class="bi bi-check-lg me-2"></i>Enregistrer le Mouvement</button>
