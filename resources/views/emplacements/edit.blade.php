...
@section('title', 'Modifier Emplacement - TPI Sidi Bennour')
...
<style>
    ...
    .form-card { background: var(--card-bg); border: 1px solid var(--card-border); border-radius: 20px; padding: 35px; max-width: 700px; margin: 0 auto; }
...
<div class="admin-wrapper">
    ...
    <main class="main-content">
        ...
        <div class="page-header">
            <h1><i class="bi bi-pencil" style="color: var(--accent-gold); margin-right: 12px;"></i>Modifier l'emplacement</h1>
            <a href="{{ route('emplacements.index') }}" class="btn-outline"><i class="bi bi-arrow-left"></i> Retour</a>
        </div>
        ...
        <div class="form-card">
            <form action="{{ route('emplacements.update', $emplacement) }}" method="POST">
                ...
                <div class="form-group">
                    <label>Salle *</label>
                    <input type="text" name="salle" class="form-control" value="{{ old('salle', $emplacement->salle) }}" required>
                </div>
                <div class="form-group">
                    <label>Armoire *</label>
                    <input type="text" name="armoire" class="form-control" value="{{ old('armoire', $emplacement->armoire) }}" required>
                </div>
                <div class="form-group">
                    <label>Étagère</label>
                    <input type="text" name="etagere" class="form-control" value="{{ old('etagere', $emplacement->etagere) }}">
                </div>
                <div class="form-group">
                    <label>Capacité maximale</label>
                    <input type="number" name="capacite_max" class="form-control" value="{{ old('capacite_max', $emplacement->capacite_max) }}">
                    <small style="color: var(--text-muted);">Laissez vide pour illimité</small>
                </div>
                <div class="form-group">
                    <label>Statut</label>
                    <select name="actif" class="form-select">
                        <option value="1" {{ $emplacement->actif ? 'selected' : '' }}>Actif</option>
                        <option value="0" {{ !$emplacement->actif ? 'selected' : '' }}>Inactif</option>
                    </select>
                </div>
                <div class="form-group">
