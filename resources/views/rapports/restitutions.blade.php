...
@section('title', 'Rapport des Restitutions - TPI Sidi Bennour')
...
<div class="admin-wrapper">
    ...
    <main class="main-content">
        <div class="top-bar">
            <div class="breadcrumb-custom"><a href="{{ route('dashboard') }}">Accueil</a> / <a href="{{ route('rapports.index') }}">Rapports</a> / <span>Restitutions</span></div>
            <div class="top-bar-right"><form action="{{ route('logout') }}" method="POST">@csrf<button type="submit" class="top-icon-btn"><i class="bi bi-box-arrow-right"></i></button></form></div>
        </div>
        <div class="page-header"><div><a href="{{ route('rapports.index') }}"><i class="bi bi-arrow-left"></i> Retour</a><h1><i class="bi bi-arrow-return-left" style="color: var(--accent-gold);"></i> Rapport des Restitutions</h1></div><div><button onclick="window.print()" class="btn-outline"><i class="bi bi-printer"></i> Imprimer</button></div></div>
        <div class="filter-card">
            <form method="GET" action="{{ route('rapports.restitutions') }}" class="row g-3">
                <div class="col-md-4"><label>Statut</label><select name="statut" class="filter-select"><option value="">Tous</option>@foreach($statuts as $stat)<option value="{{ $stat }}" {{ request('statut') == $stat ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $stat)) }}</option>@endforeach</select></div>
                <div class="col-md-4 d-flex align-items-end"><button type="submit" class="btn-gold w-100"><i class="bi bi-search"></i> Filtrer</button></div>
            </form>
        </div>
        <div class="table-card">
            <table class="table-dark-custom"><thead><tr><th>Demandeur</th><th>CIN</th><th>Pièce</th><th>Motif</th><th>Statut</th><th>Date</th></tr></thead>
            <tbody>@forelse($restitutions as $rest)<tr><td style="color:var(--text-light);font-weight:600">{{ $rest->demandeur_nom }}</td><td>{{ $rest->demandeur_cin ?? 'N/A' }}</td><td>{{ $rest->piece->reference ?? 'N/A' }}</td><td>{{ Str::limit($rest->motif_demande, 40) }}</td><td><span class="status-badge status-{{ $rest->statut }}">{{ ucfirst(str_replace('_', ' ', $rest->statut)) }}</span></td><td>{{ $rest->created_at->format('d/m/Y') }}</td></tr>@empty<tr><td colspan="6" style="text-align:center;">Aucune restitution trouvée</td></tr>@endforelse</tbody>
            </table>
            <div class="pagination">{{ $restitutions->appends(request()->query())->links() }}</div>
        </div>
    </main>
</div>
@endsection