...
@section('title', 'Rapport des Inventaires - TPI Sidi Bennour')
...
<div class="admin-wrapper">
    ...
    <main class="main-content">
        <div class="top-bar"><div class="breadcrumb-custom"><a href="{{ route('dashboard') }}">Accueil</a> / <a href="{{ route('rapports.index') }}">Rapports</a> / <span>Inventaires</span></div><div class="top-bar-right"><form action="{{ route('logout') }}" method="POST">@csrf<button type="submit" class="top-icon-btn"><i class="bi bi-box-arrow-right"></i></button></form></div></div>
        <div class="page-header"><div><a href="{{ route('rapports.index') }}"><i class="bi bi-arrow-left"></i> Retour</a><h1><i class="bi bi-clipboard-check" style="color: var(--accent-gold);"></i> Rapport des Inventaires</h1></div><div><button onclick="window.print()" class="btn-outline"><i class="bi bi-printer"></i> Imprimer</button></div></div>
        <div class="filter-card"><form method="GET" action="{{ route('rapports.inventaires') }}" class="row g-3"><div class="col-md-4"><label>Statut</label><select name="statut" class="filter-select"><option value="">Tous</option>@foreach($statuts as $stat)<option value="{{ $stat }}" {{ request('statut') == $stat ? 'selected' : '' }}>{{ ucfirst($stat) }}</option>@endforeach</select></div><div class="col-md-4 d-flex align-items-end"><button type="submit" class="btn-gold w-100"><i class="bi bi-search"></i> Filtrer</button></div></form></div>
        <div class="table-card">
            <table class="table-dark-custom"><thead><tr><th>Référence</th><th>Type</th><th>Réalisé par</th><th>Statut</th><th>Date planifiée</th><th>Pièces trouvées</th></tr></thead>
            <tbody>@forelse($inventaires as $inv)<tr><td style="color:var(--text-light);font-weight:600">{{ $inv->reference }}</td><td>{{ ucfirst($inv->type) }}</td><td>{{ $inv->realisePar->name ?? 'N/A' }}</td><td><span class="status-badge status-{{ $inv->statut }}">{{ ucfirst($inv->statut) }}</span></td><td>{{ $inv->date_planifiee->format('d/m/Y') }}</td><td>{{ $inv->pieces_trouvees ?? 0 }}/{{ $inv->pieces_attendues ?? 0 }}</td></tr>@empty<tr><td colspan="6" style="text-align:center;">Aucun inventaire trouvé</td></tr>@endforelse</tbody>
            </table>
            <div class="pagination">{{ $inventaires->appends(request()->query())->links() }}</div>
        </div>
    </main>
</div>
@endsection