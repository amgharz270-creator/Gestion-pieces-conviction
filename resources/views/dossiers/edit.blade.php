...
@section('title', 'Modifier Dossier - TPI Sidi Bennour')
...
<style>
    ...
    /* Info Badge */
    .info-badge { background: rgba(201, 162, 39, 0.1); border: 1px solid rgba(201, 162, 39, 0.3); color: var(--accent-gold); padding: 15px 25px; border-radius: 12px; margin-bottom: 25px; font-size: 0.95rem; display: flex; align-items: center; gap: 12px; }
    .info-badge strong { color: var(--text-light); }
    ...
    .form-control-dark:disabled { opacity: 0.5; cursor: not-allowed; }
    ...
    <main class="main-content">
        ...
        <div class="page-header">
            <div>
                ...
                <h1><i class="bi bi-pencil-square" style="color: var(--accent-gold); margin-right: 12px;"></i>Modifier le Dossier</h1>
                <p>Modifiez les informations du dossier judiciaire</p>
            </div>
        </div>

        <div class="info-badge">
            <i class="bi bi-info-circle-fill"></i>
            Dossier <strong>{{ $dossier->numero_dossier }}</strong> | Créé le {{ $dossier->created_at->format('d/m/Y') }}
        </div>
        ...
        <div class="form-card">
            <form action="{{ route('dossiers.update', $dossier) }}" method="POST" id="dossierForm">
                @csrf
                @method('PUT')

                <div class="form-card-header">
                    <h3><i class="bi bi-info-circle"></i> Informations Générales</h3>
                    <p>Modifiez les informations de base du dossier</p>
                </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group-dark">
                            ...
                            <select name="type_affaire" 
                                    ...
                                    required>
                                <option value="penale" {{ old('type_affaire', $dossier->type_affaire) == 'penale' ? 'selected' : '' }}>Pénale</option>
                                <option value="civile" {{ old('type_affaire', $dossier->type_affaire) == 'civile' ? 'selected' : '' }}>Civile</option>
                                <option value="commerciale" {{ old('type_affaire', $dossier->type_affaire) == 'commerciale' ? 'selected' : '' }}>Commerciale</option>
                                <option value="administrative" {{ old('type_affaire', $dossier->type_affaire) == 'administrative' ? 'selected' : '' }}>Administrative</option>
                            </select>
                        </div>
                    ...
                    <div class="col-12">
                        ...
                        <div class="input-group-dark">
                            ...
                            <textarea name="parties" 
                                      ...
                                      required>{{ old('parties', $dossier->parties) }}</textarea>
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group-dark">
                            ...
                            <select name="juge_id" class="form-select-dark {{ $errors->has('juge_id') ? 'is-invalid' : '' }}">
                                ...
                                <option value="{{ $juge->id }}" {{ old('juge_id', $dossier->juge_id) == $juge->id ? 'selected' : '' }}>
                                    {{ $juge->name }} ({{ $juge->role }})
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group-dark">
                            ...
                            <select name="statut" 
                                ...
                                <option value="juge" {{ old('statut', $dossier->statut) == 'juge' ? 'selected' : '' }}>Jugé</option>
                                <option value="appel" {{ old('statut', $dossier->statut) == 'appel' ? 'selected' : '' }}>En Appel</option>
                                <option value="cassation" {{ old('statut', $dossier->statut) == 'cassation' ? 'selected' : '' }}>Cassation</option>
                                <option value="clos" {{ old('statut', $dossier->statut) == 'clos' ? 'selected' : '' }}>Clos</option>
                            </select>
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group-dark">
                            ...
                            <input type="date" 
                                   ...
                                   value="{{ old('date_cloture', $dossier->date_cloture ? $dossier->date_cloture->format('Y-m-d') : '') }}">
                        </div>
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <textarea name="observations" 
                                  ...
                                  rows="4" 
                                  placeholder="Notes, observations ou remarques sur le dossier...">{{ old('observations', $dossier->observations) }}</textarea>
            ...
            <div class="delete-warning">
                <h4><i class="bi bi-exclamation-triangle-fill"></i> Zone de Danger</h4>
                <p>La suppression de ce dossier est irréversible. Toutes les pièces associées seront également affectées.</p>
                <form action="{{ route('dossiers.destroy', $dossier) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer définitivement ce dossier ?');">
                    @csrf
                    @method('DELETE')
