...
@section('title', 'Nouvelle Restitution - TPI Sidi Bennour')
...
<style>
    ...
    .form-select-dark { background: rgba(255,255,255,0.05); border: 2px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 14px 18px; color: #fff; font-size: 0.95rem; width: 100%; font-family: 'Poppins', sans-serif; cursor: pointer; }
    .form-select-dark:focus { border-color: var(--accent-gold); box-shadow: 0 0 0 0.2rem rgba(201, 162, 39, 0.15); outline: none; }
    ...
    .section-divider { display: flex; align-items: center; gap: 15px; margin: 40px 0 30px; color: var(--accent-gold); font-weight: 600; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 1px; }
...
<div class="admin-wrapper">
    ...
    <main class="main-content">
        ...
        <div class="page-header">
            <div>
                <a href="{{ route('restitutions.index') }}"><i class="bi bi-arrow-left"></i> Retour a la liste</a>
                <h1><i class="bi bi-arrow-return-left" style="color: var(--accent-gold); margin-right: 12px;"></i>Nouvelle Demande de Restitution</h1>
            </div>
        </div>
        ...
        <div class="form-card">
            <form action="{{ route('restitutions.store') }}" method="POST">
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <select name="piece_id" class="form-select-dark {{ $errors->has('piece_id') ? 'is-invalid' : '' }}" required>
                            ...
                            <option value="{{ $piece->id }}" {{ old('piece_id') == $piece->id ? 'selected' : '' }}>
                                {{ $piece->reference }} - {{ Str::limit($piece->description, 30) }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Type de Demandeur <span class="required">*</span></label>
                        <select name="type_demandeur" class="form-select-dark {{ $errors->has('type_demandeur') ? 'is-invalid' : '' }}" required>
                            <option value="">Choisir...</option>
                            <option value="victime" {{ old('type_demandeur') == 'victime' ? 'selected' : '' }}>Victime</option>
                            <option value="prevenu" {{ old('type_demandeur') == 'prevenu' ? 'selected' : '' }}>Prévenu</option>
                            <option value="tiers" {{ old('type_demandeur') == 'tiers' ? 'selected' : '' }}>Tiers</option>
                            <option value="avocat" {{ old('type_demandeur') == 'avocat' ? 'selected' : '' }}>Avocat</option>
                            <option value="heritier" {{ old('type_demandeur') == 'heritier' ? 'selected' : '' }}>Héritier</option>
                        </select>
                        @if($errors->has('type_demandeur'))<div class="invalid-feedback">{{ $errors->first('type_demandeur') }}</div>@endif
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Nom du Demandeur <span class="required">*</span></label>
                        <input type="text" name="demandeur_nom" class="form-control-dark {{ $errors->has('demandeur_nom') ? 'is-invalid' : '' }}" value="{{ old('demandeur_nom') }}" placeholder="Nom complet" required>
                        @if($errors->has('demandeur_nom'))<div class="invalid-feedback">{{ $errors->first('demandeur_nom') }}</div>@endif
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">CIN du Demandeur</label>
                        <input type="text" name="demandeur_cin" class="form-control-dark" value="{{ old('demandeur_cin') }}" placeholder="Ex: AB123456">
                    </div>

                    <div class="section-divider"><i class="bi bi-file-text"></i> Jugement</div>

                    <div class="col-md-6">
                        <label class="form-label">Référence du Jugement</label>
                        <input type="text" name="jugement_reference" class="form-control-dark" value="{{ old('jugement_reference') }}" placeholder="Ex: 2026/123">
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Date du Jugement</label>
                        <input type="date" name="date_jugement" class="form-control-dark" value="{{ old('date_jugement') }}">
                    </div>

                    <div class="col-12">
                        <label class="form-label">Motif de la Demande <span class="required">*</span></label>
                        <textarea name="motif_demande" class="form-control-dark {{ $errors->has('motif_demande') ? 'is-invalid' : '' }}" rows="3" placeholder="Décrivez le motif de la demande de restitution..." required>{{ old('motif_demande') }}</textarea>
                        @if($errors->has('motif_demande'))<div class="invalid-feedback">{{ $errors->first('motif_demande') }}</div>@endif
                    </div>

                    <div class="col-12 mt-4">
                        <button type="submit" class="btn-gold"><i class="bi bi-check-lg me-2"></i>Soumettre la Demande</button>
                        <a href="{{ route('restitutions.index') }}" class="btn-outline ms-3">Annuler</a>
                    </div>
                </div>
            </form>
        </div>
