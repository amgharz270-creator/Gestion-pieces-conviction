...
@section('title', 'Modifier Restitution - TPI Sidi Bennour')
...
<div class="admin-wrapper">
    ...
    <main class="main-content">
        ...
        <div class="page-header">
            <h1><i class="bi bi-pencil" style="color: var(--accent-gold); margin-right: 12px;"></i>Modifier la restitution #{{ $restitution->id }}</h1>
            ...
            <form action="{{ route('restitutions.update', $restitution) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="statut" class="form-select" required>
                        <option value="en_attente" {{ old('statut', $restitution->statut) == 'en_attente' ? 'selected' : '' }}>⏳ En attente</option>
                        <option value="approuvee" {{ old('statut', $restitution->statut) == 'approuvee' ? 'selected' : '' }}>✅ Approuvée</option>
                        <option value="refusee" {{ old('statut', $restitution->statut) == 'refusee' ? 'selected' : '' }}>❌ Refusée</option>
                        <option value="effectuee" {{ old('statut', $restitution->statut) == 'effectuee' ? 'selected' : '' }}>📦 Effectuée</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>Observations</label>
                    <textarea name="observations" class="form-control" rows="4" placeholder="Observations supplémentaires...">{{ old('observations', $restitution->observations) }}</textarea>
                </div>
                <div class="form-group">
